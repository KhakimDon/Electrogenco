...
export default {
    ...
    mounted() {
        setTimeout(() => {
            this.fallback = this.useBlogStore.ventilation_motors[4].prod
            this.$i18n.locale = 'en'
        }, 200);
    ...
    methods: {
        filter() {
            ...
            if (event.target.dataset.filterby == "OUTPUT POWER") {
                let filtered = this.useBlogStore.ventilation_motors[4].prod.filter(item => item[this.$t("lang")].specifications.Power == event.target.dataset.value)
