    ...
    data() {
        return {
            filters: [
                ...
                {
                    en: "NUMBER OF IMPELLER",
                    ru: "КОЛИЧЕСТВО ИМПЛЕЛЛЕРОВ",
                    uz: "IMPELLER SONI",
                    items: {
                        en: ["Single-Impeller",
                            "Two-impeller"],
    ...
    methods: {
        ...
        filter() {
            ...
            if (event.target.dataset.filterby == "NUMBER OF PHASE" ||
                event.target.dataset.filterby == "FAZA SONI" ||
                event.target.dataset.filterby == "КОЛИЧЕСТВО ФАЗЫ"
            ) {
                let filtered = this.massiv.filter(item => item.en.specifications.Phase == event.target.dataset.value)
                ...
                event.target.dataset.filterby == "КОЛИЧЕСТВО ИМПЛЕЛЛЕРОВ" ||
                event.target.dataset.filterby == "IMPELLER SONI"
            ) {
                let filtered = this.massiv.filter(item => item.en.specifications.Impeller == event.target.dataset.value)
                this.fallback = filtered
            }
        ...
        <div id="breadcrumbs" class="max-w-[1250px] mx-auto">
            <div class="text-sm breadcrumbs mt-[15px]">
                <ul>
                    ...
                    <li class="roboto-medium text-[#1A85FF]">{{ $t('1.7') }}</li>
                </ul>
            </div>
        </div>

        <div class="max-w-[1250px] sm:mt-[20px] mx-auto mt-[50px]">
            ...
            <div class="flex items-start gap-[25px] sm:flex-col sm:px-[20px] sm:gap-[0px]">
                ...
                <div v-if="this.finV" class="flex-1 flex gap-[25px] flex-wrap">
                    ...
                    <Cards @click="$router.push(`/water-pumps/${item.id}`)" :title="item[$t('lang')].title"
