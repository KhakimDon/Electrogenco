...
export default {
    ...
    mounted() {
        setTimeout(() => {
            this.fallback = this.useBlogStore.water_pumps_for_desert_cooler[1].prod
            this.$i18n.locale = 'en'
        }, 200);
    ...
    methods: {
        filter() {
            ...
            if (event.target.dataset.filterby == "CONDUCTOR MATERIAL") {
                let filtered = this.useBlogStore.water_pumps_for_desert_cooler[1].prod.filter(item => item[this.$t("lang")].specifications.Material == event.target.dataset.value)
                this.fallback = filtered
            }
            else if (event.target.dataset.filterby == "APPLICATION") {
                let filtered = this.useBlogStore.water_pumps_for_desert_cooler[1].prod.filter(item => item[this.$t("lang")].specifications.Application == event.target.dataset.value)
                this.fallback = filtered
            }
