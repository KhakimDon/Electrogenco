...
export default {
    ...
    mounted() {
        setTimeout(() => {
            this.fallback = this.useBlogStore.ventilation_motors[0].prod
             this.$i18n.locale = 'en'
        }, 200);
    ...
    methods: {
        filter(){
            ...
            if(event.target.dataset.filterby == "OUTPUT POWER"){
                let filtered = this.useBlogStore.ventilation_motors[0].prod.filter(item => item[this.$t("lang")].specifications.Power.toLowerCase().split(' ').join('') == event.target.dataset.value.toLowerCase().split(' ').join(''))
