...
export default {
    ...
    mounted() {
        setTimeout(() => {
            this.fallback = this.useBlogStore.ventilation_motors[2].prod
            this.$i18n.locale = 'en'
        }, 200);
    ...
    methods: {
        filter() {
            ...
            if (event.target.dataset.filterby == "OUTPUT POWER") {
                let filtered = this.useBlogStore.ventilation_motors[2].prod.filter(item => item[this.$t("lang")].specifications.Power == event.target.dataset.value)
                this.fallback = filtered
            }
            else if (event.target.dataset.filterby == "CONDUCTOR MATERIAL") {
                let filtered = this.useBlogStore.ventilation_motors[2].prod.filter(item => item[this.$t("lang")].specifications.ConductorMaterial == event.target.dataset.value)
                this.fallback = filtered
            }
            else if (event.target.dataset.filterby == "NUMBER OF SPEED") {
                let filtered = this.useBlogStore.ventilation_motors[2].prod.filter(item => item[this.$t("lang")].specifications.Speed == event.target.dataset.value)
                this.fallback = filtered
            }
